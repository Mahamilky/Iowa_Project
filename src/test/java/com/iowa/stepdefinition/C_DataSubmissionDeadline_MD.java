...
public class C_DataSubmissionDeadline_MD extends BaseClassIowa {
	...
	public void user_click_on_plus_add_button() throws Throwable {
		...
		waitforclicktoelement(pom.getDSD().getAddButton());
	...
	public void user_entering_the_period() throws Throwable {
		...
		inputOnElement(pom.getDSD().getSelectperiod(), "Common");
	...
	public void user_select_the_start_date_from_calender() throws Throwable {
		...
		MonthAndYearPicker(pom.getDSD().getMonthFieldCalenderStart(), pom.getDSD().getPreviousbuttonCalenderStart(),
				"April", pom.getDSD().getChoosestartdate());
		log.info("user successfully selected Start date and month");
	}
	...
	public void user_select_the_end_date_from_calender() throws Throwable {
		...
		MonthAndYearPicker(pom.getDSD().getMonthFieldCalenderEnd(), pom.getDSD().getNextbuttonCalenderEnd(), "June",
				pom.getDSD().getChooseenddate());
		log.info("user succeffully selected end date and month ");
	}
	...
	public void user_choose_the_files_to_be_submitted_by_clicking_on_checkboxes() throws Throwable {
		...
		scroll(pom.getDSD().getClickEmployee());
		clickOnElement(pom.getDSD().getClickEmployee());
		clickOnElement(pom.getDSD().getClickPosition());
	...
	public void user_choose_start_date_from_calender() throws Throwable {
		...
		log.debug("user choosing start date from calender under Reporting year tab");
		clickOnElement(pom.getDSD().getPeriodstartdate());
		MonthAndYearPicker(pom.getDSD().getMonthCalenderPeriodStart(), pom.getDSD().getPreviousButtonPeriodCalenderStart(),
				"April", pom.getDSD().getChoosePeriodStartDate());
		log.info("user successfully entered start date and month");
	}

	@When("^user chosse end date from calender$")
	public void user_chosse_end_date_from_calender() throws Throwable {

		log.debug("user choosing end date from calender under Reporting Year tab");
		clickOnElement(pom.getDSD().getPeriodenddate());
		MonthAndYearPicker(pom.getDSD().getMonthCalenderPeriodEnd(), pom.getDSD().getNextButtonPeriodCalenderEnd(), "June",
				pom.getDSD().getChoosePeriodEndDate());
		log.info("user successfully entered end date and month");
